...
TMP_DIR="$(mktemp -d /tmp/mee-no-rust-build-prefers-ir.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

out_wat="$TMP_DIR/control.wat"
out_err="$TMP_DIR/control.err"

MEE_NO_RUST_BUILD=1 "$ROOT_DIR/mee" build "$ROOT_DIR/tests/ir_subset_control_flow.mee" --emit=wat -o "$out_wat" 2>"$out_err"
