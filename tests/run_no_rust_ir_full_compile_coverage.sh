...
TMP_DIR="$(mktemp -d /tmp/mee-ir-full-norust.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

failed=0
while IFS= read -r src; do
  out="$TMP_DIR/$(basename "$src" .mee).wat"
  if ! MEE_NO_RUST=1 "$ROOT_DIR/mee" build "$ROOT_DIR/$src" --emit=wat --toolchain=ir -o "$out" >/dev/null 2>"$TMP_DIR/err.log"; then
    failed=1
    echo "[FAIL] $src"
