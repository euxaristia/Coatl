...
TMP_DIR="$(mktemp -d /tmp/mee-auto-asm-full-norust.XXXXXX)"
...
while IFS= read -r src; do
  ...
  if ! MEE_NO_RUST=1 "$ROOT_DIR/mee" build "$ROOT_DIR/$src" --emit=asm --toolchain=auto -o "$out" >/dev/null 2>"$TMP_DIR/err.log"; then
    failed=1
    echo "[FAIL] $src"
