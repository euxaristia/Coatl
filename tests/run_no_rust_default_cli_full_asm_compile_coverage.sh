...
TMP_DIR="$(mktemp -d /tmp/coatl-default-asm-norust.XXXXXX)"
...
while IFS= read -r src; do
  ...
  if ! COATL_NO_RUST=1 "$ROOT_DIR/coatl" build "$ROOT_DIR/$src" --emit=asm -o "$out" >/dev/null 2>"$TMP_DIR/err.log"; then
    failed=1
    echo "[FAIL] $src"
