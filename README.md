...
In strict no-Rust mode, struct programs currently require `--toolchain=auto` (selfhost path); the subset `--toolchain=ir` frontend does not yet parse struct syntax.
...
- In strict no-Rust mode, `--toolchain=ir` currently supports scalar/control-flow/I/O subset coverage, not struct syntax.
