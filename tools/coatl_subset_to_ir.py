...
def main():
    ...
    # But since we have the integrated coatl.py script, we just call it.
    cmd = [coatl_bin, "build", input_file, "-o", output_file]
    # To force IR emission, we use the .ir extension which our coatl.py script handles
    if not output_file.endswith(".ir"):
        # If the user didn't specify .ir, they probably still want IR if they called this tool.
        # But our coatl.py script determines action by extension.
