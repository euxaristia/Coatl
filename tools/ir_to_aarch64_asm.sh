...
MODULE="/tmp/coatl-ir-to-aarch64.wat"

usage() {
  cat <<'EOF' >&2
usage:
  tools/ir_to_aarch64_asm.sh <input.ir> -o <output.s>
EOF
}
...
if [[ -z "$COATL_BIN" || ! -x "$COATL_BIN" ]]; then
  echo "ir_to_aarch64_asm: coatl binary not found" >&2
  exit 1
fi
if ! command -v wasmtime >/dev/null 2>&1; then
  echo "ir_to_aarch64_asm: wasmtime not found" >&2
  exit 1
fi

if [[ ! -f "$MODULE" ]]; then
  "$COATL_BIN" build "$ROOT_DIR/tools/ir_to_aarch64_asm.coatl" --emit=wat --toolchain=selfhost -o "$MODULE" >/dev/null
fi
...
if [[ "$ret" != "0" ]]; then
  echo "ir_to_aarch64_asm: lowering failed (rc=$ret)" >&2
  exit 1
fi