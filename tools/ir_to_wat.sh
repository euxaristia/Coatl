...
if [[ -z "$COATL_BIN" || ! -x "$COATL_BIN" ]]; then
  echo "ir_to_wat: coatl binary not found" >&2
  exit 1
fi
if ! command -v wasmtime >/dev/null 2>&1; then
  echo "ir_to_wat: wasmtime not found" >&2
  exit 1
fi

if [[ ! -f "$MODULE" ]]; then
  "$COATL_BIN" build "$ROOT_DIR/tools/ir_to_wat.coatl" --toolchain=selfhost -o "$MODULE" >/dev/null
fi
